...
@media (max-height: 760px) {{
    #priceChart {{ height: 455px; }}
    #volumeChart {{ height: 118px; }}
}}
@media (max-height: 690px) {{
    #priceChart {{ height: 410px; }}
    #volumeChart {{ height: 105px; }}
}}
</style>
</head>
<body>
...
const commonOptions = {{
    ...
    grid: {{
        vertLines: {{ color: '#e8e8e8', style: 0, visible: true }},
        horzLines: {{ color: '#e8e8e8', style: 0, visible: true }}
    }},
    timeScale: {{
        borderColor: '#cfcfcf',
        rightOffset: 4,
...
function setInitialRange() {{
    if (candles.length > 5) {{
        const barsToShow = Math.min(44, candles.length);
        const range = {{ from: candles.length - barsToShow - 1, to: candles.length + 3 }};
        priceChart.timeScale().setVisibleLogicalRange(range);
        volumeChart.timeScale().setVisibleLogicalRange(range);
    }}
}}
