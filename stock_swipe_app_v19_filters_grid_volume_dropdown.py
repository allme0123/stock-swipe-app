...
# Version: v19 filter tests, volume-average dropdown and adaptive price grid
#
# Run:
...
st.markdown(
    ...
    /* Keep the four chart controls in one row on narrow phones. */
    ...
    .st-key-nav_controls div[data-testid="stColumn"] {
        flex: 1 1 25% !important;
        width: 25% !important;
        min-width: 0 !important;
    }
    .st-key-nav_left button,
    .st-key-like_button button,
    .st-key-add_button button,
    .st-key-nav_right button {
...
WATCHLIST_STORE_PATH = Path(__file__).with_name("stock_swipe_watchlists.json")
LIKED_OPTION = "Liked"
CREATE_OPTION = "Create watchlist"
NONE_OPTION = "None"
RESERVED_WATCHLIST_NAMES = {LIKED_OPTION.lower(), CREATE_OPTION.lower(), NONE_OPTION.lower()}

FILTER_DEFAULT = "Filters"
FILTER_ABOVE_SMA20 = "Above 20-day SMA"
FILTER_ABOVE_SMA50 = "Above 50-day SMA"
FILTER_VOLUME_ABOVE_AVG = "Volume above 20-day average"
FILTER_SMA20_ABOVE_SMA50 = "20-day SMA above 50-day SMA"
FILTER_OPTIONS = [
    FILTER_DEFAULT,
    FILTER_ABOVE_SMA20,
    FILTER_ABOVE_SMA50,
    FILTER_VOLUME_ABOVE_AVG,
    FILTER_SMA20_ABOVE_SMA50,
]


def empty_watchlist_store() -> dict:
    return {"liked": [], "watchlists": {}}


def load_watchlist_store() -> dict:
    store = empty_watchlist_store()
    try:
        if WATCHLIST_STORE_PATH.exists():
            loaded = json.loads(WATCHLIST_STORE_PATH.read_text(encoding="utf-8"))
            if isinstance(loaded, dict):
                liked = loaded.get("liked", [])
                watchlists = loaded.get("watchlists", {})
                if isinstance(liked, list):
                    store["liked"] = list(dict.fromkeys(str(t).upper().strip() for t in liked if str(t).strip()))
                if isinstance(watchlists, dict):
                    cleaned = {}
                    for name, members in watchlists.items():
                        clean_name = str(name).strip()
                        if not clean_name or clean_name.lower() in RESERVED_WATCHLIST_NAMES:
                            continue
                        if not isinstance(members, list):
                            members = []
                        cleaned[clean_name] = list(
                            dict.fromkeys(str(t).upper().strip() for t in members if str(t).strip())
                        )
                    store["watchlists"] = cleaned
    except Exception:
        # A damaged or unavailable store should not prevent the chart app from loading.
        return empty_watchlist_store()
    return store


def save_watchlist_store(store: dict) -> None:
    payload = {
        "liked": list(dict.fromkeys(store.get("liked", []))),
        "watchlists": {
            str(name): list(dict.fromkeys(members))
            for name, members in store.get("watchlists", {}).items()
        },
    }
    temp_path = WATCHLIST_STORE_PATH.with_suffix(".tmp")
    temp_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    temp_path.replace(WATCHLIST_STORE_PATH)


def request_filter_change() -> None:
    st.session_state.idx = 0


def request_watchlist_change() -> None:
    requested = st.session_state.get("watchlist_selector", NONE_OPTION)
    if requested == CREATE_OPTION:
        st.session_state.show_create_watchlist_dialog = True
        st.session_state.watchlist_selector = st.session_state.get("active_watchlist", NONE_OPTION)
        return
    st.session_state.active_watchlist = requested
    st.session_state.idx = 0


@st.dialog("Create watchlist")
def create_watchlist_dialog() -> None:
    name = st.text_input("Watchlist name", max_chars=40, key="new_watchlist_name")
    left, right = st.columns(2)
    with left:
        cancel = st.button("Cancel", use_container_width=True, key="cancel_create_watchlist")
    with right:
        create = st.button("Create", type="primary", use_container_width=True, key="confirm_create_watchlist")
    ...
    if create:
        ...
        existing_lower = {n.lower() for n in store["watchlists"]}
        if not clean_name:
            st.error("Enter a watchlist name.")
            return
        if clean_name.lower() in RESERVED_WATCHLIST_NAMES:
            st.error("That name is reserved. Choose another name.")
            return
        if clean_name.lower() in existing_lower:
            st.error("A watchlist with that name already exists.")
            return
        store["watchlists"][clean_name] = []
        save_watchlist_store(store)
        st.session_state.active_watchlist = clean_name
        st.session_state.watchlist_selector = clean_name
        st.session_state.idx = 0
        st.session_state.pop("new_watchlist_name", None)
        st.session_state.flash_message = f'Created watchlist "{clean_name}".'
        st.rerun()


@st.dialog("Add to watchlist")
def add_to_watchlist_dialog(ticker: str) -> None:
    store = load_watchlist_store()
    names = list(store["watchlists"].keys())
    if not names:
        st.info("Create a watchlist first using the Watchlist menu.")
        if st.button("Close", use_container_width=True, key="close_empty_add_dialog"):
            st.rerun()
        return

    target = st.selectbox("Choose watchlist", names, key="add_watchlist_target")
    already_added = ticker in store["watchlists"].get(target, [])
    if already_added:
        st.caption(f"{ticker} is already in {target}.")

    left, right = st.columns(2)
    with left:
        cancel = st.button("Cancel", use_container_width=True, key="cancel_add_watchlist")
    with right:
        add = st.button(
            "Add",
            type="primary",
    ...
    if add:
        store["watchlists"].setdefault(target, []).append(ticker)
        store["watchlists"][target] = list(dict.fromkeys(store["watchlists"][target]))
        save_watchlist_store(store)
        st.session_state.flash_message = f"Added {ticker} to {target}."
        st.rerun()
...
def add_indicators(d: pd.DataFrame) -> pd.DataFrame:
    ...
    d["VolumeSMA20"] = d["Volume"].rolling(20).mean()
...
<style>
html, body {{ margin:0; padding:0; width:100%; background:#fff; font-family:Arial, Helvetica, sans-serif; overscroll-behavior:auto; -webkit-tap-highlight-color:transparent; overflow:hidden; }}
...
.chart-mode-select {{ height:19px; min-width:70px; padding:0 18px 0 5px; border:1px solid #b9b9b9; border-radius:4px; background:#fff; color:#555; font-size:11px; }}
.volume-average-select {{ min-width:118px; max-width:145px; }}
...
#priceCanvas {{ width:100%; height:343px; display:block; touch-action:pan-y pinch-zoom; }}
...
@media (max-height:760px) {{ #priceCanvas {{ height:313px; }} #volumeCanvas {{ height:142px; }} }}
@media (max-height:690px) {{ #priceCanvas {{ height:284px; }} #volumeCanvas {{ height:128px; }} }}
</style></head><body>
<div class="wrap">
...
<div class="toolbar"><span>Daily</span><select id="chartModeSelect" class="chart-mode-select" aria-label="Chart type"><option value="candle">Candle</option><option value="line">Line</option></select><span>Drag / pinch zoom</span></div>
...
<div class="panel"><div class="panel-title"><span class="legend-volume"></span><span>Volume</span><select id="volumeAverageSelect" class="chart-mode-select volume-average-select" aria-label="Volume average period"><option value="5">5-day average</option><option value="10">10-day average</option><option value="14">14-day average</option><option value="20">20-day average</option><option value="30">30-day average</option><option value="50">50-day average</option><option value="100">100-day average</option><option value="200">200-day average</option><option id="currentCustomVolumeOption" value="" hidden></option><option value="custom">Custom…</option></select></div><canvas id="volumeCanvas"></canvas></div>
...
const volumeAverageSelect = document.getElementById('volumeAverageSelect');
const currentCustomVolumeOption = document.getElementById('currentCustomVolumeOption');
const standardVolumeAverageDays = [5,10,14,20,30,50,100,200];
...
let volumeAverageDays=14;
try {{
    const savedDays=Number(localStorage.getItem('stockSwipeVolumeAverageDays'));
    if(Number.isInteger(savedDays) && savedDays>=1 && savedDays<=252) volumeAverageDays=savedDays;
}} catch(e) {{}}
...
function calculateVolumeAverage() {{
    ...
    for(let i=0;i<rows.length;i++) {{
        const current=Number(rows[i].volume)||0;
        rollingTotal += current;
        if(i>=volumeAverageDays) rollingTotal -= Number(rows[i-volumeAverageDays].volume)||0;
        rows[i].volumeAverage = i>=volumeAverageDays-1 ? rollingTotal/volumeAverageDays : null;
    }}
}}
...
function niceStep(range, targetTicks=5) {{
    ...
    const multiplier=n<=1?1:n<=2?2:n<=2.5?2.5:n<=5?5:10;
    return multiplier*pow;
}}
function priceTickTarget(visibleBars) {{
    if(visibleBars<=12) return 12;
    if(visibleBars<=24) return 10;
    if(visibleBars<=45) return 8;
    if(visibleBars<=90) return 7;
    if(visibleBars<=160) return 6;
    return 5;
}}
...
function priceGridValues(minP,maxP,tickStep) {{
    const values=[];
    const first=Math.ceil((minP-1e-10)/tickStep);
    const last=Math.floor((maxP+1e-10)/tickStep);
    for(let k=first;k<=last;k++) values.push(k*tickStep);
    return values;
}}
function drawPriceGrid(ctx,left,right,top,bottom,minP,maxP,tickStep,gridValues) {{
    const y=val=>bottom-(val-minP)/(maxP-minP)*(bottom-top);
    ctx.strokeStyle='#e5e5e5'; ctx.lineWidth=0.65;
    for(const val of gridValues) {{
        const yy=Math.round(y(val))+0.5;
        ctx.beginPath(); ctx.moveTo(left,yy); ctx.lineTo(right,yy); ctx.stroke();
    }}
    const verticalDivisions=view.count<=16?8:view.count<=32?6:view.count<=80?5:4;
    for(let i=0;i<=verticalDivisions;i++) {{
        const x=Math.round(left+(right-left)*i/verticalDivisions)+0.5;
...
function drawPrice() {{
    ...
    const targetTicks=priceTickTarget(view.count);
    const tickStep=niceStep(maxP-minP,targetTicks);
    minP=Math.floor(minP/tickStep)*tickStep;
    maxP=Math.ceil(maxP/tickStep)*tickStep;
    const gridValues=priceGridValues(minP,maxP,tickStep);
    const y=val=>bottom-(val-minP)/(maxP-minP)*(bottom-top);
    drawPriceGrid(pctx,left,right,top,bottom,minP,maxP,tickStep,gridValues);
    ...
    for(const val of gridValues) pctx.fillText(fmtPrice(val,tickStep),right+5,y(val));
...
function drawVolume() {{
    ...
    const avgVals=volData.map(x=>x.volumeAverage).filter(x=>x!==null&&Number.isFinite(x));
    ...
    // User-selected simple moving average through the volume bars.
...
function syncVolumeAverageSelect() {{
    if(standardVolumeAverageDays.includes(volumeAverageDays)) {{
        currentCustomVolumeOption.hidden=true;
        currentCustomVolumeOption.value='';
        currentCustomVolumeOption.textContent='';
    }} else {{
        currentCustomVolumeOption.hidden=false;
        currentCustomVolumeOption.value=String(volumeAverageDays);
        currentCustomVolumeOption.textContent=volumeAverageDays+'-day average';
    }}
    volumeAverageSelect.value=String(volumeAverageDays);
}}
...
function drawAll(){{chartModeSelect.value=chartMode;syncVolumeAverageSelect();sma20Button.textContent=showSma?'20-DAY SMA: ON':'20-DAY SMA: OFF';sma20Button.classList.toggle('on',showSma);drawPrice();drawVolume();persistView();}}
...
chartModeSelect.addEventListener('change',()=>{{chartMode=chartModeSelect.value==='line'?'line':'candle';drawAll();}});
volumeAverageSelect.addEventListener('change',()=>{{
    let selected=volumeAverageSelect.value;
    if(selected==='custom') {{
        const entered=window.prompt('Number of days for the volume average (1-252):',String(volumeAverageDays));
        if(entered===null) {{ syncVolumeAverageSelect(); return; }}
        selected=entered.trim();
    }}
    const parsed=Number(selected);
    if(!Number.isInteger(parsed) || parsed<1 || parsed>252) {{
        window.alert('Enter a whole number from 1 to 252.');
        syncVolumeAverageSelect();
        return;
    }}
    volumeAverageDays=parsed;
    try{{localStorage.setItem('stockSwipeVolumeAverageDays',String(volumeAverageDays));}}catch(e){{}}
    calculateVolumeAverage();
    drawAll();
}});
...
if "active_watchlist" not in st.session_state:
    st.session_state.active_watchlist = NONE_OPTION

if "watchlist_selector" not in st.session_state:
    st.session_state.watchlist_selector = NONE_OPTION

if "filter_selector" not in st.session_state:
    st.session_state.filter_selector = FILTER_DEFAULT
...
try:
    ...
    st.selectbox(
        ...
        FILTER_OPTIONS,
        key="filter_selector",
        on_change=request_filter_change,
        label_visibility="collapsed",
    )
    selected_filter = st.session_state.filter_selector
    ...
    for t in tickers:
        ...
        dd = add_indicators(ticker_data)
        last = dd.iloc[-1]

        if selected_filter == FILTER_ABOVE_SMA20 and not (
            pd.notna(last["SMA20"]) and last["Close"] > last["SMA20"]
        ):
            continue

        if selected_filter == FILTER_ABOVE_SMA50 and not (
            pd.notna(last["SMA50"]) and last["Close"] > last["SMA50"]
        ):
            continue

        if selected_filter == FILTER_VOLUME_ABOVE_AVG and not (
            pd.notna(last["VolumeSMA20"]) and last["Volume"] > last["VolumeSMA20"]
        ):
            continue

        if selected_filter == FILTER_SMA20_ABOVE_SMA50 and not (
            pd.notna(last["SMA20"])
    ...
    store = load_watchlist_store()
    custom_watchlists = list(store["watchlists"].keys())
    watchlist_options = [LIKED_OPTION, CREATE_OPTION, NONE_OPTION] + custom_watchlists

    if st.session_state.active_watchlist not in watchlist_options or st.session_state.active_watchlist == CREATE_OPTION:
        st.session_state.active_watchlist = NONE_OPTION
    if st.session_state.watchlist_selector not in watchlist_options or st.session_state.watchlist_selector == CREATE_OPTION:
        st.session_state.watchlist_selector = st.session_state.active_watchlist

    st.selectbox(
        "Watchlist",
        watchlist_options,
        key="watchlist_selector",
        on_change=request_watchlist_change,
        label_visibility="collapsed",
    )

    if st.session_state.pop("show_create_watchlist_dialog", False):
        create_watchlist_dialog()

    active_watchlist = st.session_state.active_watchlist
    if active_watchlist == LIKED_OPTION:
        allowed = set(store["liked"])
        visible_tickers = [t for t in filtered if t in allowed]
    elif active_watchlist == NONE_OPTION:
        visible_tickers = filtered
    else:
        allowed = set(store["watchlists"].get(active_watchlist, []))
        visible_tickers = [t for t in filtered if t in allowed]

    if not visible_tickers:
        if active_watchlist == LIKED_OPTION:
            st.info("The Liked watchlist is empty. Select None, then tap the like button on a chart.")
        elif active_watchlist == NONE_OPTION:
            st.warning("No tickers are available.")
        else:
            st.info(f'"{active_watchlist}" is empty or its tickers do not pass the current filters.')
        st.stop()

    st.session_state.idx %= len(visible_tickers)
    ticker = visible_tickers[st.session_state.idx]
    liked = ticker in store["liked"]

    # The liked state is white with a grey border; all inactive controls use chart grey.
    like_background = "#ffffff" if liked else "#858585"
    like_colour = "#666666" if liked else "#ffffff"
    st.markdown(
        f"""
        <style>
        .st-key-like_button button {{
            background: {like_background} !important;
            color: {like_colour} !important;
    ...
    with st.container(key="nav_controls"):
        col_left, col_like, col_add, col_right = st.columns(4, gap="small")

        with col_left:
            go_left = st.button("◀", use_container_width=True, key="nav_left", help="Previous chart")

        with col_like:
            toggle_like = st.button(
                "Liked" if liked else "Like",
                use_container_width=True,
                key="like_button",
                help="Remove like" if liked else "Like this chart",
            )

        with col_add:
            add_to_list = st.button("+", use_container_width=True, key="add_button", help="Add to watchlist")

        with col_right:
            go_right = st.button("▶", use_container_width=True, key="nav_right", help="Next chart")

    if go_left:
        st.session_state.idx = (st.session_state.idx - 1) % len(visible_tickers)
        st.rerun()
    ...
    if toggle_like:
        store = load_watchlist_store()
        if ticker in store["liked"]:
            store["liked"] = [t for t in store["liked"] if t != ticker]
        else:
            store["liked"].append(ticker)
            store["liked"] = list(dict.fromkeys(store["liked"]))
        save_watchlist_store(store)
        st.rerun()

    if add_to_list:
        st.session_state.show_add_watchlist_dialog = True

    if st.session_state.pop("show_add_watchlist_dialog", False):
        add_to_watchlist_dialog(ticker)
