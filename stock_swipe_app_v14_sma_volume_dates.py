...
# Version: v14 SMA and adaptive date updates
#
# Run:
...
.scroll-gap {{ height:20px; margin:0 10px 4px 10px; background:#fff; touch-action:pan-y; display:flex; align-items:center; justify-content:center; color:#c0c0c0; font-size:11px; }}
...
<div class="hint">Double tap toggles candle / line</div>
...
function chartBounds(canvas, axisPad=54) {{
    return {{ left: 9, right: canvas.clientWidth - axisPad, top: 10, bottom: canvas.clientHeight - 43 }};
}}
...
function dateTicks(meta) {{
    ...
    let step;
    if (visibleBars <= 14) step = 1;
    else if (visibleBars <= 32) step = 2;
    else if (visibleBars <= 65) step = 5;
    else if (visibleBars <= 130) step = 10;
    else if (visibleBars <= 260) step = 20;
    else step = 40;
...
function drawPrice() {{
    ...
    for(const t of dateTicks(meta)){{ const x=xForIdx(t.idx,left,right,meta)+step/2; if(x<left+13||x>right-13)continue; const a=t.label.split('/'); if(a.length===3){{ pctx.fillText(a[0].padStart(2,'0'),x,bottom+3); pctx.fillText(a[1].padStart(2,'0'),x,bottom+14); pctx.fillText(a[2].slice(-2),x,bottom+25); }} }}
}}
...
function drawAll(){{document.getElementById('chartModeLabel').textContent=chartMode==='line'?'Line':'Candle';drawPrice();drawVolume();persistView();}}
...
function end(e){{
    ...
    const now=Date.now(); if(now-lastTap<300){{chartMode=chartMode==='line'?'candle':'line';drawAll();}} lastTap=now;
}}
...
try:
    ...
    sma_label = "20-DAY SMA: ON" if st.session_state.show_sma20 else "20-DAY SMA: OFF"
    if st.button(sma_label, key="main_sma20_button", type="primary" if st.session_state.show_sma20 else "secondary", use_container_width=True):
        st.session_state.show_sma20 = not st.session_state.show_sma20
        st.rerun()
    ...
    html = chart_html(d, ticker, show_sma=st.session_state.show_sma20)
