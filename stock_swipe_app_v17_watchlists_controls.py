...
# Version: v17 watchlists, likes and four-button navigation
#
# Run:
...
st.markdown(
    ...
    .st-key-like_button.liked button {
...
#priceCanvas {{ width:100%; height:381px; display:block; touch-action:pan-y pinch-zoom; }}
...
@media (max-height:760px) {{ #priceCanvas {{ height:348px; }} #volumeCanvas {{ height:142px; }} }}
@media (max-height:690px) {{ #priceCanvas {{ height:316px; }} #volumeCanvas {{ height:128px; }} }}
</style></head><body>
<div class="wrap">
...
try:
    ...
    # Streamlit gives each keyed widget a stable st-key-* wrapper. Add a class-like
    # descendant selector through the active state so the liked button becomes white.
    like_background = "#ffffff" if liked else "#d40000"
    like_colour = "#d40000" if liked else "#ffffff"
    st.markdown(
        f"""
        <style>
        .st-key-like_button button {{
    ...
    col_left, col_like, col_add, col_right = st.columns(4)

    with col_left:
        go_left = st.button("◀", type="primary", use_container_width=True, key="nav_left", help="Previous chart")

    with col_like:
        toggle_like = st.button(
            "👍︎",
            type="secondary" if liked else "primary",
            use_container_width=True,
            key="like_button",
    ...
    with col_add:
        add_to_list = st.button("+", type="primary", use_container_width=True, key="add_button", help="Add to watchlist")

    with col_right:
        go_right = st.button("▶", type="primary", use_container_width=True, key="nav_right", help="Next chart")
