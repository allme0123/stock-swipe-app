...
# Version: v7 CommSec-style canvas layout
#
# Run:
...
.scroll-gap {{ height:4px; background:#fff; touch-action:pan-y; }}
...
let view = {{ count: Math.min(76, rows.length), end: rows.length - 1, minCount: 10, maxCount: Math.min(360, rows.length) }};
...
function drawPrice() {{
 ...
 const pad=(maxP-minP)*0.14 || maxP*0.02 || 1; minP-=pad; maxP+=pad; const y=val=>bottom-(val-minP)/(maxP-minP)*(bottom-top); const step=(right-left)/data.length; const bodyW=Math.max(0.8, Math.min(step*0.58, 15));
...
function drawVolume() {{
 ...
 const left=7,right=w-54,top=8,bottom=h-27; drawGrid(vctx,left,right,top,bottom,3); const maxV=Math.max(...data.map(x=>x.volume),1); const step=(right-left)/data.length; const barW=Math.max(0.7, Math.min(step*0.52, 13));
...
function end(e){{ if(e.touches&&e.touches.length>0){{ if(e.touches.length<2)pinch=null; return; }} pinch=null; drag=null; const now=Date.now(); if(now-lastTap<300){{ view.count=Math.min(76,rows.length); view.end=rows.length-1; drawAll(); }} lastTap=now; }}
