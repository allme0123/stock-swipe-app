...
# Version: v8 CommSec-style free-scroll canvas layout
#
# Run:
...
.panel {{ margin:0 10px 13px 10px; border:1px solid #bdbdbd; background:#fff; box-sizing:border-box; }}
...
.scroll-gap {{ height:32px; margin:0 10px 13px 10px; background:#fff; touch-action:pan-y; display:flex; align-items:center; justify-content:center; color:#c0c0c0; font-size:11px; }}
...
<div class="scroll-gap">scroll area</div>
...
let view = {{ count: Math.min(95, rows.length + 30), end: rows.length - 1 + 10, minCount: 8 }};
...
function chartBounds(canvas, axisPad=54) {{
    return {{ left: 9, right: canvas.clientWidth - axisPad, top: 10, bottom: canvas.clientHeight - 31 }};
}}
function maxZoomOut(canvas) {{
    const b = chartBounds(canvas);
    // Keep at least about 2 pixels per bar so candles/volume bars retain gaps.
    const pixelLimited = Math.floor((b.right - b.left) / 2.05);
    return Math.max(80, Math.min(rows.length + 90, pixelLimited));
}}
...
function dateTicks(meta, maxTicks=5) {{
    const ticks = [];
    const span = Math.max(1, meta.end - meta.start);
    const raw = span / maxTicks;
    const steps = [1,2,5,10,20,40,60,90,120];
...
function drawPrice() {{
    ...
    const b=chartBounds(priceCanvas,54); const left=b.left,right=b.right,top=b.top,bottom=b.bottom;
    ...
    if(showSma){{ for(const k of ['sma20','sma50','sma200']){{ const vals=priceData.map(x=>x[k]).filter(x=>x!==null&&Number.isFinite(x)); if(vals.length){{ minP=Math.min(minP,...vals); maxP=Math.max(maxP,...vals); }} }} }}
    const pad=(maxP-minP)*0.13 || maxP*0.02 || 1; minP-=pad; maxP+=pad;
    const y=val=>bottom-(val-minP)/(maxP-minP)*(bottom-top);
    const step=(right-left)/view.count;
    const bodyW=Math.max(1.0, Math.min(step*0.46, 10.5));

    pctx.fillStyle='#666'; pctx.font='11px Arial'; pctx.textAlign='left'; pctx.textBaseline='middle';
    for(let i=0;i<=5;i++){{ const val=maxP-(maxP-minP)*i/5; pctx.fillText(fmtPrice(val),right+5,top+(bottom-top)*i/5); }}
    ...
    data.forEach(bar=>{{
        const x=xForIdx(bar.idx,left,right,meta)+step/2; if(x<left-4||x>right+4) return;
        ...
        pctx.strokeStyle=col; pctx.lineWidth=0.9; pctx.beginPath(); pctx.moveTo(Math.round(x)+0.5,yh); pctx.lineTo(Math.round(x)+0.5,yl); pctx.stroke();
        if(up){{ pctx.fillStyle='#fff'; pctx.strokeStyle=col; pctx.lineWidth=0.9; pctx.fillRect(x-bodyW/2,bodyTop,bodyW,bh); pctx.strokeRect(x-bodyW/2,bodyTop,bodyW,bh); }}
    ...
    if(ly>=top&&ly<=bottom){{
        ...
        pctx.fillStyle='#1679a5'; const label=fmtPrice(last.close); pctx.fillRect(right+2,ly-10,49,20); pctx.fillStyle='#fff'; pctx.font='bold 11px Arial'; pctx.textAlign='center'; pctx.textBaseline='middle'; pctx.fillText(label,right+26.5,ly);
    }}

    // CommSec-like dates under the candlestick box. They adapt as the zoom level changes.
    pctx.fillStyle='#6e6e6e'; pctx.font='11px Arial'; pctx.textAlign='center'; pctx.textBaseline='top';
    for (const t of dateTicks(meta,5)) {{ const x=xForIdx(t.idx,left,right,meta)+step/2; if(x>=left+16 && x<=right-16) pctx.fillText(t.label,x,bottom+8); }}
}}

function drawVolume() {{
    setupCanvas(volumeCanvas,vctx);
    const w=volumeCanvas.clientWidth,h=volumeCanvas.clientHeight;
    const b=chartBounds(volumeCanvas,54); const left=b.left,right=b.right,top=8,bottom=h-20;
    ...
    const maxV=Math.max(...volData.map(x=>x.volume),1);
    const step=(right-left)/view.count; const barW=Math.max(0.85, Math.min(step*0.42, 9.5));
...
function start(e){{
    if(e.touches&&e.touches.length===2){{ const a=e.touches[0],b=e.touches[1]; const dist=Math.hypot(a.clientX-b.clientX,a.clientY-b.clientY); const midX=(a.clientX+b.clientX)/2; pinch={{dist,count:view.count,end:view.end,center:pointerToIndex(midX)}}; drag=null; e.preventDefault(); return; }}
    const t=e.touches?e.touches[0]:e; drag={{x:t.clientX,end:view.end}};
}}
function move(e){{
    if(pinch&&e.touches&&e.touches.length===2){{ const a=e.touches[0],b=e.touches[1]; const dist=Math.hypot(a.clientX-b.clientX,a.clientY-b.clientY); const rect=priceCanvas.getBoundingClientRect(); const bnd=chartBounds(priceCanvas); const midX=(a.clientX+b.clientX)/2; const frac=(midX-rect.left-bnd.left)/Math.max(1,bnd.right-bnd.left); view.count=pinch.count*pinch.dist/Math.max(10,dist); view.end=pinch.center+view.count*(1-frac)-1; clampView(priceCanvas); drawAll(); e.preventDefault(); return; }}
    if(drag){{ const t=e.touches?e.touches[0]:e; const dx=t.clientX-drag.x; const rect=priceCanvas.getBoundingClientRect(); const bnd=chartBounds(priceCanvas); const pxPerBar=Math.max(1,(bnd.right-bnd.left)/view.count); view.end=drag.end-dx/pxPerBar; clampView(priceCanvas); drawAll(); e.preventDefault(); }}
}}
function end(e){{
    if(e.touches&&e.touches.length>0){{ if(e.touches.length<2)pinch=null; return; }}
    pinch=null; drag=null; const now=Date.now(); if(now-lastTap<300){{ view.count=Math.min(95, rows.length + 30); view.end=rows.length - 1 + 10; clampView(priceCanvas); drawAll(); }} lastTap=now;
}}
...
try:
    ...
    components.html(html, height=835, scrolling=True)
