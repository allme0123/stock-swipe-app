...
# Version: v15 SMA and adaptive date updates
#
# Run:
...
function chartBounds(canvas, axisPad=27) {{
...
function drawPrice() {{
    ...
    const b=chartBounds(priceCanvas,27); const left=b.left,right=b.right,top=b.top,bottom=b.bottom;
    ...
    for(const t of dateTicks(meta)){{ const x=xForIdx(t.idx,left,right,meta)+step/2; if(x<left+25||x>right-25)continue; const a=t.label.split('/'); if(a.length===3){{ const label=a[0].padStart(2,'0')+'/'+a[1].padStart(2,'0')+'/'+a[2].slice(-2); pctx.fillText(label,x,bottom+8); }} }}
}}

function drawVolume() {{
    setupCanvas(volumeCanvas,vctx);
    const w=volumeCanvas.clientWidth,h=volumeCanvas.clientHeight;
    const b=chartBounds(volumeCanvas,27); const left=b.left,right=b.right,top=8,bottom=h-20;
