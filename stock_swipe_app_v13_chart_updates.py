...
# Version: v13 requested chart updates
#
# Run:
...
function drawPrice() {{
    ...
    for(const t of dateTicks(meta,5)){{ const x=xForIdx(t.idx,left,right,meta)+step/2; if(x<left+16||x>right-16)continue; const a=t.label.split('/'); if(a.length===3){{ pctx.fillText(String(Number(a[0])),x,bottom+3); pctx.fillText(String(Number(a[1])),x,bottom+14); pctx.fillText('20'+a[2],x,bottom+25); }} }}
}}
...
with st.sidebar:
    ...
    st.caption("The 20-day SMA control is above the chart.")
...
try:
    ...
    st.session_state.show_sma20 = st.toggle("20-day SMA", value=st.session_state.show_sma20, key="main_sma20_toggle")
