...
# Version: v18 mobile controls, filters and editable chart settings
#
# Run:
...
FILTER_NEAR_HIGH = "Within 10% of 52-week high"
FILTER_UPTREND = "20 > 50 > 200 trend"
FILTER_BOTH = "Both filters"
FILTER_OPTIONS = [FILTER_DEFAULT, FILTER_NEAR_HIGH, FILTER_UPTREND, FILTER_BOTH]
...
.volume-average-button {{ margin:0; padding:0; border:0; background:transparent; color:#555; font:inherit; text-decoration:underline; text-underline-offset:2px; cursor:pointer; }}
...
<div class="panel"><div class="panel-title"><span class="legend-volume"></span><span>Volume ·</span><button id="volumeAverageButton" class="volume-average-button" type="button">14-day average</button></div><canvas id="volumeCanvas"></canvas></div>
...
const volumeAverageButton = document.getElementById('volumeAverageButton');
...
function drawAll(){{chartModeSelect.value=chartMode;volumeAverageButton.textContent=volumeAverageDays+'-day average';sma20Button.textContent=showSma?'20-DAY SMA: ON':'20-DAY SMA: OFF';sma20Button.classList.toggle('on',showSma);drawPrice();drawVolume();persistView();}}
...
volumeAverageButton.addEventListener('click',()=>{{
...
try:
    ...
    only_near_highs = selected_filter in {FILTER_NEAR_HIGH, FILTER_BOTH}
    only_uptrend = selected_filter in {FILTER_UPTREND, FILTER_BOTH}
