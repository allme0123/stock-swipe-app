# stock_swipe_app_v23_exact_price_narrow_ticker_locked_chart_multifilters.py
# Mobile-first ASX stock table with sticky columns, sortable headers,
# inline expandable charts, watchlists and action menus.
# v24: expanded charts remain scrollable and only one ticker chart can be open at a time.
#
# Run:
#   pip install streamlit pandas openpyxl yfinance
#   streamlit run stock_swipe_app_v23_exact_price_narrow_ticker_locked_chart_multifilters.py
...
.app {
    ...
    background: #fff;
    /* The hosted Streamlit toolbar overlays the top of the component on phones.
       This safe space keeps the controls and sticky column headings visible. */
    padding-top: 104px;
}
.controlbar {
    flex: 0 0 auto;
...
.filter-caret { font-size: 12px; color: #666; }
.filter-menu {
    display: none;
    position: absolute;
    top: calc(100% + 4px);
...
.filter-option input { width: 18px; height: 18px; margin: 0; accent-color: #666; }
.clear-filters {
    width: 100%;
    height: 40px;
    border: 0;
...
.code-col { width: 77px; min-width: 77px; max-width: 77px; text-align: left; }
...
td.action-col.menu-open {
    /* Each sticky action cell creates its own stacking layer. Raise the active
       cell above later rows so its menu cannot be painted underneath them. */
...
@media (max-width: 620px) {
    ...
    .controlbar select, .filter-button { height: 36px; font-size: 13px; }
    ...
    .code-col { width: 70px; min-width: 70px; max-width: 70px; }
...
<div class="app">
    <div class="controlbar">
        <div id="filterControl" class="filter-control">
            <button id="filterButton" class="filter-button" type="button" aria-haspopup="true" aria-expanded="false">
                <span id="filterButtonLabel">Filters</span><span class="filter-caret">▼</span>
            </button>
            <div id="filterMenu" class="filter-menu" role="menu">
                <label class="filter-option"><input type="checkbox" value="aboveSma20">Above 20-day SMA</label>
                <label class="filter-option"><input type="checkbox" value="aboveSma50">Above 50-day SMA</label>
                <label class="filter-option"><input type="checkbox" value="volumeAboveAverage">Volume above 20-day average</label>
                <label class="filter-option"><input type="checkbox" value="sma20AboveSma50">20-day SMA above 50-day SMA</label>
...
function updateFilterButton() {
    ...
    filterButtonLabel.textContent = count ? `Filters (${count})` : 'Filters';
...
function closeExpandedChart() {
    tableShell.classList.remove('chart-open');
    state.expandedTicker = null;
    activeChart = null;
}
function applySelectedFilters() {
    state.filters = Array.from(filterMenu.querySelectorAll('input[type="checkbox"]:checked')).map(input => input.value);
    closeExpandedChart();
    updateFilterButton();
    render();
}
function filteredStocks() {
    let result = allStocks.filter(stock => {
        if (state.filters.includes('aboveSma20') && !stock.aboveSma20) return false;
        if (state.filters.includes('aboveSma50') && !stock.aboveSma50) return false;
        if (state.filters.includes('volumeAboveAverage') && !stock.volumeAboveAverage) return false;
        if (state.filters.includes('sma20AboveSma50') && !stock.sma20AboveSma50) return false;
...
function closeMenus(exceptTicker=null) {
    document.querySelectorAll('.action-menu.open').forEach(menu => {
        if (menu.dataset.actionMenu !== exceptTicker) {
            menu.classList.remove('open','up');
            const actionCell = menu.closest('td.action-col');
            if (actionCell) actionCell.classList.remove('menu-open');
        }
    });
}
function toggleMenu(ticker, button) {
    ...
    const actionCell = button.closest('td.action-col');
    if (actionCell) actionCell.classList.add('menu-open');
...
function toggleExpand(ticker) {
    // A single expandedTicker is the source of truth, so opening a new chart
    // automatically removes the previously expanded chart during render().
    const opening = state.expandedTicker !== ticker;
    state.expandedTicker = opening ? ticker : null;
    tableShell.classList.toggle('chart-open', opening);
    activeChart = null;
    render();
    ...
    if (opening) {
        setTimeout(() => {
            ...
            const header = tableShell.querySelector('thead');
            const headerHeight = header ? header.getBoundingClientRect().height : 49;
            const targetTop = Math.max(
                0,
                tableShell.scrollTop + (rowRect.top - shellRect.top) - headerHeight - 4
            );
            tableShell.scrollTo({top: targetTop, left: tableShell.scrollLeft, behavior:'smooth'});
        }, 80);
    }
}
...
function createChart(host, stock) {
    ...
    function drawPrice() {
        ...
        if(lastY>=top && lastY<=bottom) {
            ...
            pctx.fillText(formatPrice(last.close),right+26,lastY);
        }
...
watchlistSelect.addEventListener('change',()=>{
    ...
    state.watchlist=watchlistSelect.value;
    closeExpandedChart();
    render();
});
...
document.addEventListener('click',event=>{
    ...
    if(!event.target.closest('#filterControl')) closeFilterMenu();
});
...
with st.sidebar:
    ...
    st.caption("The main screen uses a fixed sortable header. Filters can be combined. Tap a ticker to open or close its chart.")
...
try:
    ...
    components.html(market_table_html(payload), height=940, scrolling=False)
