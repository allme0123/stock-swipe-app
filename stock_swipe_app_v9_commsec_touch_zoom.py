...
# Version: v9 CommSec-style touch-scroll + zoom-width canvas layout
#
# Run:
...
#priceCanvas {{ width:100%; height:470px; display:block; touch-action:pan-y; }}
#volumeCanvas {{ width:100%; height:155px; display:block; touch-action:pan-y; }}
...
function drawPrice() {{
    ...
    const bodyW=Math.max(1.1, Math.min(step*0.58, 54));
...
function drawVolume() {{
    ...
    const step=(right-left)/view.count; const barW=Math.max(0.9, Math.min(step*0.56, 56));
...
function start(e){{
    ...
    if(e.touches&&e.touches.length===2){{
        ...
        pinch={{dist,count:view.count,end:view.end,center:pointerToIndex(midX)}};
        drag=null;
        e.preventDefault();
        return;
    }}
    const t=e.touches?e.touches[0]:e;
    drag={{x:t.clientX,y:t.clientY,end:view.end}};
}}
function move(e){{
    if(pinch&&e.touches&&e.touches.length===2){{
        ...
        view.end=pinch.center+view.count*(1-frac)-1;
    ...
    if(drag){{
        ...
        // Let one-finger vertical movement scroll the Streamlit page anywhere on the chart,
        // including over the candle and volume canvases. Only horizontal motion pans the chart.
...
function end(e){{
    ...
    pinch=null; drag=null; gestureMode=null;
    if(wasVertical) return;
    const now=Date.now(); if(now-lastTap<300){{ view.count=Math.min(95, rows.length + 30); view.end=rows.length - 1 + 10; clampView(priceCanvas); drawAll(); }} lastTap=now;
}}
...
try:
    ...
    components.html(html, height=855, scrolling=True)
