...
# Version: v5 CommSec closest mobile layout
#
# Run:
...
.legend-price {{
    width: 14px;
    height: 9px;
    border-left: 7px solid #00b93b;
    border-right: 7px solid #e50000;
    box-sizing: border-box;
}}
.legend-volume {{
    width: 14px;
...
@media (max-height: 820px) {{
    #priceChart {{ height: 61vh; }}
    #volumeChart {{ height: 20vh; }}
}}
@media (max-height: 720px) {{
    #priceChart {{ height: 59vh; }}
    #volumeChart {{ height: 18vh; }}
}}
</style>
</head>
<body>
...
const volumes = {json.dumps(volume_data)}.map(v => ({{...v, color: v.color === '#00c23a' ? '#00a83b' : '#e50000'}}));
...
const commonOptions = {{
    ...
    grid: {{
        vertLines: {{ color: '#eeeeee', style: 0, visible: true }},
        horzLines: {{ color: '#eeeeee', style: 0, visible: true }}
    }},
    timeScale: {{
        borderColor: '#c9c9c9',
        rightOffset: 8,
...
const candleSeries = priceChart.addCandlestickSeries({{
    upColor: '#ffffff',
    downColor: '#e50000',
    borderUpColor: '#00a83b',
    borderDownColor: '#e50000',
    wickUpColor: '#00a83b',
    wickDownColor: '#e50000',
    priceLineColor: '#1681aa',
    priceLineWidth: 1,
...
function setInitialRange() {{
    if (candles.length > 5) {{
        const barsToShow = Math.min(82, candles.length);
        const range = {{ from: candles.length - barsToShow - 1, to: candles.length + 8 }};
        priceChart.timeScale().setVisibleLogicalRange(range);
        volumeChart.timeScale().setVisibleLogicalRange(range);
    }}
}}
...
// Double tap resets the visible range, similar to mobile broker chart apps.
let lastTap = 0;
document.body.addEventListener('touchend', () => {{
    const now = Date.now();
    if (now - lastTap < 300) setInitialRange();
    lastTap = now;
}}, {{ passive: true }});
