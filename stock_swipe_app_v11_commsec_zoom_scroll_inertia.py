...
# Version: v11 CommSec-style max zoom-out + native vertical inertia scroll
#
# Run:
...
#priceCanvas {{ width:100%; height:470px; display:block; touch-action:pan-y pinch-zoom; }}
#volumeCanvas {{ width:100%; height:155px; display:block; touch-action:pan-y pinch-zoom; }}
.scroll-gap {{ height:62px; margin:0 10px 13px 10px; background:#fff; touch-action:pan-y; display:flex; align-items:center; justify-content:center; color:#c0c0c0; font-size:11px; }}
...
function end(e){{
    ...
    const now=Date.now(); if(now-lastTap<300){{ view.count=Math.min(maxZoomOut(priceCanvas), rows.length + 60); view.end=rows.length - 1 + 20; clampView(priceCanvas); drawAll(); }} lastTap=now;
}}
