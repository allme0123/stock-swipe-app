...
# Version: v10 CommSec-style fixed vertical touch-scroll + compressed candle height
#
# Run:
...
function drawPrice() {{
    ...
    // Extra vertical padding compresses candle height by about 50%, closer to CommSec's calmer scale.
    const pad=(maxP-minP)*0.75 || maxP*0.06 || 1; minP-=pad; maxP+=pad;
...
function move(e){{
    ...
    if(drag){{
        ...
        // One-finger vertical movement now scrolls the iframe/page manually anywhere over the charts.
        // This avoids the canvas trapping touch events on Android/Chrome.
        if(gestureMode==='pan-y'){{
            const scrollDelta = drag.lastY - t.clientY;
